...
export default function App() {
  ...
  const handleConfirmBetSubmit = async () => {
    ...
    try {
      ...
      const isLocked = latestMatch.status !== 'upcoming' || new Date().getTime() >= (latestMatch.kickoffTimeIST ? latestMatch.kickoffTimeIST.seconds * 1000 : 0);
  ...
  const renderMatchCard = (match) => {
    ...
    const isLocked = match.status !== 'upcoming' || new Date().getTime() >= (match.kickoffTimeIST ? match.kickoffTimeIST.seconds * 1000 : 0);
